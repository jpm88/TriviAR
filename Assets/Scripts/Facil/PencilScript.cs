...
public class PencilScript : MonoBehaviour, ITrackableEventHandler {
	...
	void OnGUI() {
		if (mShowGUI) {
			...
			GUI.Box(caja, "¿Cómo se llama este objeto en inglés?");
			if (GUI.Button(boton1, "Pen")) {
				ScoreManager.RestaPuntos();
				Application.LoadLevel("Fallo");
			}
			if (GUI.Button(boton2, "Pencil")) {
				ScoreManager.SumaPuntos();
				Application.LoadLevel("Acierto");
			}
			if (GUI.Button(boton3, "Notebook")) {
				ScoreManager.RestaPuntos();
				Application.LoadLevel("Fallo");
			}
			if (GUI.Button(boton4, "Rubber")) {
				ScoreManager.RestaPuntos();
				Application.LoadLevel("Fallo");
			}
		}
